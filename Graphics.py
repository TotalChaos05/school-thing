...
# The <Graphics> library was written by 
# Mr. John Schram (last updated 10/31/22)
...
def drawCircle(centerX,centerY,r):
   if r < 0:
      ...
      sleep(2)
      raise ValueError(
         "radius argument for drawCircle/fillCircle cannot be negative.")
   x = getX(centerX)
   y = getY(centerY)
   setheading(0)
   penup()
...
def drawPolygon(points):   
   if len(points) < 6:
      ...
      sleep(2)
      raise TypeError(
         "When using drawPolygon/fillPolygon you must have at least 6 int arguments.")
   elif len(points) % 2 != 0:
      update()
      sleep(2) 
      raise TypeError(
         "When using drawPolygon/fillPolygon you must have an even # of int arguments.")
   else:
...
def drawArc(centerX,centerY,hr,vr,start,finish=None):   
   ...
   if hr <= 0 and vr <= 0:
      ...
      sleep(2)
      raise ValueError(
         "Arc radii need to be greater than 0.")      
   if (finish <= start):
      finish = finish + 360 
...
def drawBurst(centerX,centerY,r,numLines):
   ...
   if numLines < 2:
      ...
      sleep(2)
      raise ValueError(
         "\"Bursts\" must have at least 2 lines.")
   step = 2 * pi / numLines
   rotate = pi / 2 
   while radian <= 2 * pi:
...
def drawStar(centerX,centerY,r,numPoints):
   if numPoints < 2:
      ...
      sleep(2)
      raise ValueError(
         "Stars must have at least 2 points.")
   rotate = pi / 2
   count = 0
   radius = r
...
def setColor(red,green=None,blue=None):
   if blue is None:
      ...
      if red < 0 or green < 0 or blue < 0 or red > 255 or green > 255 or blue > 255:
         ...
         sleep(2)
         raise ValueError(
            "int arguments for setColor must be between 0 and 255.")  
      colorString = "#" + decihex2(red) + decihex2(green) + decihex2(blue)
      color(colorString)      
...
def drawHeading(name,labNum):
   ...
   x = (window_width() - len(s)*23) // 2
   color("black")
   drawString(s,x,50,"Courier New",28,"bold")
